...
// ═══════════════════════════════════════════════════════════
// 5a) BUYER PROFILE (바이어 가입 프로필 입력)
// ═══════════════════════════════════════════════════════════
const BUYER_INDUSTRY_OPTIONS = [
  '자동차/부품', '전자/반도체', '기계/설비', '화학/소재', '식품/음료',
  '섬유/의류', '건설/건자재', '의료/헬스케어', '유통/물류', '기타',
];
const BUYER_SCALE_OPTIONS = [
  '1억 미만', '1~5억', '5~20억', '20~100억', '100억 이상',
];

function maskBuyerField(value, type) {
  if (!value) return '';
...
function BuyerProfilePage({ onNav, onComplete }) {
  const [form, setForm] = React.useState({
    name: '', email: '', company: '',
    industry: '', purchase_items: '', purchase_scale: '', phone: '',
    agreed: false,
  });
  ...
  return (
    <div className="auth-shell bpro-shell">
      <div className="auth-shell-bg"/>
      <div className="bpro-inner">
        <header className="auth-mini-hdr onb-mini-hdr">
          <AuthLogo size={32}/>
          <button className="onb-skip-btn" onClick={() => onComplete?.({})}>건너뛰기</button>
        </header>

        <div className="auth-steps">
          <AuthStep n={1} label="이메일·비밀번호" done/>
          <AuthStepLine done/>
          <AuthStep n={2} label="이메일 인증" done/>
          <AuthStepLine done/>
          <AuthStep n={3} label="역할 설정" done/>
          <AuthStepLine done/>
          <AuthStep n={4} label="바이어 프로필" active/>
          <AuthStepLine/>
          <AuthStep n={5} label="완료"/>
        </div>

        <div className="bpro-card">
          <div className="bpro-head">
            <h2>바이어 프로필 입력</h2>
            <p>제조공장과의 매칭 정확도를 높이기 위한 정보입니다.</p>
          </div>

          <div className="bpro-section-label">필수 항목</div>
          <div className="bpro-fields">
            <div className="bpro-field">
              <label className="bpro-label">이름 <span className="bpro-req">*</span></label>
              <input
                className="bpro-input"
                placeholder="홍길동"
                value={form.name}
            ...
            <div className="bpro-field">
              <label className="bpro-label">이메일 <span className="bpro-req">*</span></label>
              <input
                className="bpro-input"
                type="email"
            ...
            <div className="bpro-field">
              <label className="bpro-label">회사명 <span className="bpro-req">*</span></label>
              <input
                className="bpro-input"
                placeholder="(주)홍길동상사"
                value={form.company}
          ...
          <div className="bpro-section-label bpro-section-opt">선택 항목</div>
          <div className="bpro-fields">
            <div className="bpro-field">
              <label className="bpro-label">업종</label>
              <select
                className="bpro-input bpro-select"
                value={form.industry}
                onChange={e => update('industry', e.target.value)}
              >
                <option value="">업종 선택</option>
                {BUYER_INDUSTRY_OPTIONS.map(o => <option key={o} value={o}>{o}</option>)}
              </select>
            </div>
            <div className="bpro-field">
              <label className="bpro-label">주요 구매 품목</label>
              <input
                className="bpro-input"
                placeholder="예: 사출 플라스틱 부품, CNC 알루미늄 가공"
                value={form.purchase_items}
                onChange={e => update('purchase_items', e.target.value)}
              />
            </div>
            <div className="bpro-field">
              <label className="bpro-label">연간 구매 규모</label>
              <select
                className="bpro-input bpro-select"
                value={form.purchase_scale}
                onChange={e => update('purchase_scale', e.target.value)}
              >
                <option value="">규모 선택</option>
                {BUYER_SCALE_OPTIONS.map(o => <option key={o} value={o}>{o}</option>)}
              </select>
            </div>
            <div className="bpro-field">
              <label className="bpro-label">연락처</label>
              <input
                className="bpro-input"
                placeholder="010-1234-5678"
                value={form.phone}
          ...
          <div className="bpro-consent">
            <label className="bpro-chk-row">
              <input
                type="checkbox"
                className="bpro-chk"
                checked={form.agreed}
              ...
              <span>
                공장매칭 바이어로 등록하여<br/>
                <strong>전국 제조공장과 비즈니스를 시작합니다</strong>
              </span>
            </label>
            <div className="bpro-notice">
              <div>* 회사명, 업종, 구매 품목이 제조공장에 공개됩니다</div>
              <div>* 언제든지 설정에서 변경 가능합니다</div>
            </div>
          </div>

          {form.agreed && form.name && form.company && (
            <div className="bpro-preview-toggle">
              <button className="bpro-preview-btn" onClick={() => setShowPreview(v => !v)}>
                <Icon name="eye" size={13} stroke={2}/>
                제조사 노출 미리보기
              </button>
              {showPreview && (
                <div className="bpro-preview-box">
                  <div className="bpro-preview-title">제조사에게 공개되는 정보</div>
                  <dl className="bpro-preview-dl">
                    <div className="bpro-preview-row">
                      <dt>회사명</dt>
                      <dd>{maskBuyerField(form.company, 'company')}</dd>
                    </div>
                    <div className="bpro-preview-row">
                      <dt>이름</dt>
                      <dd>{maskBuyerField(form.name, 'name')}</dd>
                    </div>
                    <div className="bpro-preview-row">
                      <dt>이메일</dt>
                      <dd>{maskBuyerField(form.email, 'email')}</dd>
                    </div>
                    {form.phone && (
                      <div className="bpro-preview-row">
                        <dt>연락처</dt>
                        <dd>{maskBuyerField(form.phone, 'phone')}</dd>
                      </div>
                    )}
                    {form.industry && (
                      <div className="bpro-preview-row bpro-preview-public">
                        <dt>업종</dt>
                        <dd>{form.industry} <span className="bpro-preview-tag">공개</span></dd>
                      </div>
                    )}
                    {form.purchase_items && (
                      <div className="bpro-preview-row bpro-preview-public">
                        <dt>구매 품목</dt>
                        <dd>{form.purchase_items} <span className="bpro-preview-tag">공개</span></dd>
                      </div>
                    )}
                    {form.purchase_scale && (
                      <div className="bpro-preview-row bpro-preview-public">
                        <dt>구매 규모</dt>
                        <dd>{form.purchase_scale} <span className="bpro-preview-tag">공개</span></dd>
                      </div>
                    )}
                  </dl>
                </div>
          ...
          <button
            className="btn-primary bpro-submit"
            onClick={handleSubmit}
            disabled={!isValid || submitted}
          >
            {submitted ? '저장됨' : '프로필 저장하고 시작하기'}
            <Icon name="arrow_right" size={14} stroke={2.4}/>
          </button>
        </div>
      </div>
