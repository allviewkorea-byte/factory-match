...
// 인기 검색어
const TRENDING_SEARCHES = ['CNC + 알루미늄', '사출 + 케이스', '금형 + 자동차 부품', 'PCB + SMT', '프레스 + SUS', '도장 + 분체'];
window.MFG_DATA = {
  INDUSTRIES,
  PROCESSES,
  PRODUCTS,
...
window._dbRowToFactory = row => {
  ...
  if (row.coord_x != null && row.coord_y != null) {
    ...
    if (inKorea) {
      geoLat = lat;
      geoLng = lng;
      coord = lng > 100 ? window._latLngToSvg(lat, lng) : {
        x: lng,
        y: lat
      };
    }
